...
public class TestArmaMatrix {
  ...
  public void testDet() {
    ...
    assertEquals(_determinant, Arma.det(_testMatrix), Math.abs(_determinant) * TestData.MAX_NUMERIC_TOLERANCE);
  }
  ...
  public void testAccu() {
    assertEquals(_accu, Arma.accu(_testMatrix), TestData.MAX_NUMERIC_TOLERANCE);
  }

  /**
   * Tests {@link Arma#qr(Mat, Mat, Mat)} by assertion that <code>Q^t = Q^{-1}</code>, <code>i > j => r_{i,j} = 0</code> and <code>Q*R = X</test> holds.
   */
  @Test
  public void testQr() {
    Mat Q = new Mat();
    ...
    for (int n = 0; n < inverseQ.n_elem; n++) {
      assertEquals(tranposeQ.at(n), inverseQ.at(n), TestData.MAX_NUMERIC_TOLERANCE);
    }
    ...
    for (int n = 0; n < X.n_elem; n++) {
      assertEquals(_testMatrix.at(n), X.at(n), TestData.MAX_NUMERIC_TOLERANCE);
    }
  }

  /**
   * Tests {@link Arma#norm(Mat, int)} by comparison with {@link #_norms}.
   */
  @Test
  public void testNorm() {
    assertEquals(_norms[0], Arma.norm(_testMatrix, 1), TestData.MAX_NUMERIC_TOLERANCE);
    assertEquals(_norms[1], Arma.norm(_testMatrix, 2), TestData.MAX_NUMERIC_TOLERANCE);
  }
}
