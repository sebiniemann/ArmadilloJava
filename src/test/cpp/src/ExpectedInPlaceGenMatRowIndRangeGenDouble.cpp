/*******************************************************************************
 * Copyright 2013-2014 Sebastian Niemann <niemann@sra.uni-hannover.de>.
 * 
 * Licensed under the MIT License (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * 
 * http://opensource.org/licenses/MIT
 * 
 * Developers:
 *   Sebastian Niemann - Lead developer
 *   Daniel Kiechle - Unit testing
 ******************************************************************************/
#include <Expected.hpp>
using armadilloJava::Expected;

#include <iostream>
using std::cout;
using std::endl;

#include <utility>
using std::pair;

#include <armadillo>
using arma::Mat;
using arma::span;

#include <InputClass.hpp>
using armadilloJava::InputClass;

#include <Input.hpp>
using armadilloJava::Input;

namespace armadilloJava {
  class ExpectedInPlaceGenMatRowIndRangeGenDouble : public Expected {
    public:
      ExpectedInPlaceGenMatRowIndRangeGenDouble() {
        cout << "Compute ExpectedInPlaceGenMatRowIndRangeGenDouble(): " << endl;

          vector<vector<pair<string, void*>>> inputs = Input::getTestParameters({
            InputClass::GenMat,
            InputClass::RowIndRange,
            InputClass::GenDouble
          });

          for (vector<pair<string, void*>> input : inputs) {
            _fileSuffix = "";

            int n = 0;
            for (pair<string, void*> value : input) {
              switch (n) {
                case 0:
                  _fileSuffix += value.first;
                  _genMat = *static_cast<Mat<double>*>(value.second);
                  break;
                case 1:
                  _fileSuffix += "," + value.first;
                  _rowIndRange = *static_cast<span*>(value.second);
                  break;
                case 2:
                  _fileSuffix += "," + value.first;
                  _genDouble = *static_cast<double*>(value.second);
                  break;
              }
              ++n;
            }

            cout << "Using input: " << _fileSuffix << endl;

            _copyOfGenMat = _genMat;
            _copyOfRowIndRange = _rowIndRange;
            _copyOfGenDouble = _genDouble;

            expectedMatRowsPlus();

            _genMat = _copyOfGenMat;
            _rowIndRange = _copyOfRowIndRange;
            _genDouble = _copyOfGenDouble;
            expectedMatRowsMinus();

            _genMat = _copyOfGenMat;
            _rowIndRange = _copyOfRowIndRange;
            _genDouble = _copyOfGenDouble;
            expectedMatRowsTimes();

            _genMat = _copyOfGenMat;
            _rowIndRange = _copyOfRowIndRange;
            _genDouble = _copyOfGenDouble;
            expectedMatRowsDivide();
          }

          cout << "done." << endl;
        }

    protected:
      Mat<double> _genMat;
      Mat<double> _copyOfGenMat;

      span _rowIndRange;
      span _copyOfRowIndRange;

      double _genDouble;
      double _copyOfGenDouble;

      void expectedMatRowsPlus() {
        if(_rowIndRange.b >= _genMat.n_rows) {
          return;
        }

        cout << "- Compute expectedMatRowsPlus() ... ";

        _genMat.rows(_rowIndRange.a, _rowIndRange.b) += _genDouble;
        save<double>("Mat.rowsPlus", _genMat);

        cout << "done." << endl;
      }

      void expectedMatRowsMinus() {
        if(_rowIndRange.b >= _genMat.n_rows) {
          return;
        }

        cout << "- Compute expectedMatRowsMinus() ... ";

        _genMat.rows(_rowIndRange.a, _rowIndRange.b) -= _genDouble;
        save<double>("Mat.rowsMinus", _genMat);

        cout << "done." << endl;
      }

      void expectedMatRowsTimes() {
        if(_rowIndRange.b >= _genMat.n_rows) {
          return;
        }

        cout << "- Compute expectedMatRowsTimes() ... ";

        _genMat.rows(_rowIndRange.a, _rowIndRange.b) *= _genDouble;
        save<double>("Mat.rowsTimes", _genMat);

        cout << "done." << endl;
      }

      void expectedMatRowsDivide() {
        if(_rowIndRange.b >= _genMat.n_rows) {
          return;
        }

        cout << "- Compute expectedMatRowsDivide() ... ";

        _genMat.rows(_rowIndRange.a, _rowIndRange.b) /= _genDouble;
        save<double>("Mat.rowsDivide", _genMat);

        cout << "done." << endl;
      }
  };
}
