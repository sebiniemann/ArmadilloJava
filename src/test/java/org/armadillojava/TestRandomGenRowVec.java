...
public class TestRandomGenRowVec extends TestClass {
  ...
  public void testShuffle() throws IOException {
    ...
    for(int n = 2; n <= _random; n++) {
      result = (result.times(n)).plus(Arma.shuffle(_genRowVec)).elemDivide(n + 1);
    }
    assertMatEquals(result.minus(load("shuffle")), Arma.zeros(_genRowVec.n_rows, _genRowVec.n_cols), 1);
  }
